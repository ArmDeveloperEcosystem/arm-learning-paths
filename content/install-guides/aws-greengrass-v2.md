...
You'll create an AWS Identify and Access Management (IAM) user, then install AWS IoT Greengrass and prepare it for use on your device. 
...
You'll need to use an AWS account has permissions to create access keys and assign roles to those keys. While you can use an account with administrator access, it's a best practice to use an IAM user with only the necessary permissions. 

You'll therefore create a Greengrass installation IAM user that has just enough permissions to also setup and deploy devices into the Greengrass environment.

You'll use the installation IAM user to create and save an access key and secret access key for AWS CLI access. 
...
After creating the IAM, you can create access keys for AWS CLI access.
...
Set environment variables to allow AWS IoT Greengrass to connect with your AWS account. Replace the access key and secret access key with the values that you saved earlier:
...
{{% notice Note %}}
If your target device is running the newest versions of Ubuntu (25.x, 26.x), the new distros use a rust-based `sudo` command by default. This rust-based command doesn't support the `-E` option to export the entire environment to the `sudo` session. 
...
{{% notice Note %}}
Greengrass doesn't run as a `root` user on your device. The `ggc_user` user belonging to the `ggc_group` group will be the service account for running AWS IoT Greengrass components on your device.
{{% /notice %}}
