...
If the image appears frozen, the Edge Impulse Linux Runner has finished playing the video and is waiting for a `restart` command. To replay the video, see [Send commands through AWS IoT Core](/learning-paths/embedded-and-microcontrollers/edge_impulse_greengrass/running#send-commands-through-aws-iot-core).
