...
The board is the TI [AM62L EVM](https://www.ti.com/tool/TMDS62LEVM). It boots from a micro-SD card, so you need one and a way to write it. The micro-USB connector **J7** is the serial console. It shows up on the host as four serial ports, and the console is the one named SoC UART0, usually the first. Power comes from a USB-C supply that supports Power Delivery (PD), on **J17** or **J19**.
...
`swig` and `python3-dev` let U-Boot build `pylibfdt`, `libssl-dev` gives `mkimage` its RSA code, and `libgnutls28-dev` and `uuid-dev` are for `mkeficapsule`, which TI's configuration builds even though you don't use it. `device-tree-compiler` provides `dtc`, which reads the public key node out of a blob when you build U-Boot.

`mtools` edits the SD card image without root, `xz-utils` unpacks TI's card image, `openssl` creates the keys, `curl` downloads the two TI files, and `picocom` is the serial terminal.

`u-boot-tools` is optional: it installs Ubuntu's own `mkimage` as a fallback, but you build `mkimage` from TI's U-Boot tree when you [create the signing keys and sign the image](/learning-paths/embedded-and-microcontrollers/zephyr-signed-fit-uboot-cortex-a/4-sign-zephyr/).
...
The first group of variables points into the TI SDK, which you install next. `UBOOT_SRC` is the U-Boot source, `PREBUILT` holds the firmware that runs before U-Boot, and `CROSS` is the cross compiler prefix. `SYSROOT` is the library path the SDK's compiler needs through `--sysroot`. `UBOOT_OUT` receives the U-Boot build, `KEYS` holds the signing keys, and `FIT` holds the signed images.

`ZEPHYR_ADDR` is where Zephyr is built to run, and `FIT_ADDR` is where U-Boot puts the signed image while it checks it. They must not overlap, because U-Boot copies Zephyr from one to the other.
...
You use three things from the TI Processor SDK Linux for AM62Lx and nothing from Linux itself: the U-Boot source tree `ti-u-boot-2026.01+git`, the prebuilt firmware that runs before U-Boot, and the aarch64 GCC toolchain (GCC 15.3.0).

The prebuilt firmware is TF-A as `bl1.bin` and `bl31.bin`, OP-TEE 4.10 as `bl32.bin`, and TI's system firmware in the `ti-sysfw` directory. These binaries are the ones inside TI's own image, so you don't build them.
...
`--mode unattended` accepts every default, and `--prefix` installs into `$TISDK` without root.
...
The `.wic.xz` file is TI's complete SD card image, compressed with `xz`. You don't boot Linux from it. When you [prepare the SD card and boot the board](/learning-paths/embedded-and-microcontrollers/zephyr-signed-fit-uboot-cortex-a/6-boot-the-board/), you keep its first partition as a template and replace only the files inside, because the AM62L boot ROM is picky about that partition's layout.
...
The compressed file is about 1.3 GB. Leave it compressed: that page reads the part it needs straight out of `xz`.

## What you've accomplished and what's next

Your host now has the build packages and the TI SDK: U-Boot source, prebuilt firmware and toolchain. `$WORK/env.sh` holds every path, and TI's card image is on disk as the boot partition template.
