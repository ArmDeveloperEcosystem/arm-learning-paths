...
![Diagram showing a zoomed-in neural network node with connections from the previous layer. The diagram illustrates how z1 is calculated as a dot product of input values (x) and connection weights (w), demonstrating the fundamental matrix multiplication operation that KleidiAI optimizes in AI inference.#center](neural-node-pic.webp "Zoomed-in neural network node.")
