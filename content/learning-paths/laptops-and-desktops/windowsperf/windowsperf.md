...
## WindowsPerf Cheatsheet (PMU Counting Examples)
...
## WindowsPerf Cheatsheet (PMU Sampling Examples)
...
> Hint: Same workflow can be wrapped with `wperf record` command, see example below.
...
## WindowsPerf Cheatsheet (SPE Examples)

Double-dash operator can be used with SPE as well:

```command
wperf record -e arm_spe_0/ld=1/ -c 1 -– path\to\python_d.exe -c 10**10**100
```
