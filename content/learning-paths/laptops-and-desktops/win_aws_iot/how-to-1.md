...
## Introduction
AWS IoT Core is a managed cloud platform that enables connected devices to easily and securely interact with cloud applications and other devices. It can support billions of devices and trillions of messages, processing and routing these messages to AWS endpoints and other devices reliably and securely. This functionality allows applications to continuously track and communicate with all your devices, even when they aren’t connected.
...
In this learning path, you will create the application that emulates the weather station and sends hypothesized sensor readings (like temperature, humidity) to the AWS cloud.
...
## Create the project
You will now create a Node.js project. Open the Command Prompt and type the following commands:
...
To proceed further, implement the weather station emulator. Within the AWS.WeatherStation folder, create a new file named emulator.js. Then, modify the file using the following code snippet:
...
The code imports necessary modules: common.js, emulator.js, and aws-iot-device-sdk. The first two modules were previously described, while the latter is a library that enables devices to interact with AWS IoT services.

Then, the module creates the AWS IoT device connection, which establishes a connection to AWS IoT by creating a device instance with configuration settings provided by common.awsConnectionInfo.
