...
Depending on the Zena CSS version, multiple firmware and image files might be present. Rather than searching the Yocto metadata immediately, it is useful to first review the Zena CSS boot process documentation, particularly the image layout described in the [Boot process documentation](https://arm-zena-css.docs.arm.com/en/v2.2.1/design/boot_process.html#images-layout).
...
`do_deploy` is not a default task. A recipe that needs it must inherit the `deploy` class, define `do_deploy()`, and add the task at the required point, for example with `addtask deploy after do_install`.

Zena CSS also contains [image-signing](https://arm-zena-css.docs.arm.com/en/v2.2.1/design/boot_process.html#images-layout) tasks that are specific to the platform firmware flow.
