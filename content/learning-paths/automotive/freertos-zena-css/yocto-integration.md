...
Select **RD-Aspen Cfg2**, **Virtualization**, and then **FreeRTOS** under **Safety Island RTOS**:
...
The output identifies `virtualization-image` as the target and `fvp-rd-aspen` as the machine. The target comes from selecting **Virtualization** in `kas menu`. These values provide two investigation paths:

- Follow `virtualization-image` to understand what the requested build produces
...
```console
find ../sw-ref-stack -name 'virtualization-image.bb'
```

The search finds `../sw-ref-stack/yocto/meta-arm-auto-solutions/recipes-core/images/virtualization-image.bb`. This recipe builds the Linux virtualization image, but it doesn't directly select the SI CL1 firmware.
...
```console
kas shell -c 'bitbake -e virtualization-image' > virtualization-environment.txt
...
`do_deploy` is not part of the standard task set. The Zephyr recipe receives deployment support from its inherited classes. The FreeRTOS recipe must follow the same pattern by inheriting `deploy`, defining `do_deploy()`, and adding it to the task graph after `do_install`.
...
The initial compiler dependency introduces another interesting failure. The version of `gcc-aarch64-none-elf-native` provided by Zena CSS is GCC 13, which doesn't recognize `-mcpu=cortex-r82ae`.
...
With the GCC 13 compatibility patch and recipe available, BitBake can build `freertos-demos-cl1` directly. However, the temporary edit to `arm-auto-solutions.yml` is not an appropriate permanent interface. The machine defaults still need a controlled user selection that adds `freertos` to `DISTRO_FEATURES`.
...
```console
kas shell -c "bitbake -e virtualization-image | grep '^DISTRO_FEATURES='"
```
...
Build the complete virtualization stack with the FreeRTOS selection:

```console
kas build
```

The build runs `virtualization-image` and its firmware dependencies. A successful build ends with a task summary reporting that all tasks succeeded.
