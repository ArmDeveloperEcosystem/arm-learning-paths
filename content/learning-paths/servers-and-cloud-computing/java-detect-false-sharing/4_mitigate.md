...
Edit `FalseSharingDemo.java` so that it contains the following complete program:
...
Record with the same Perf binary, CPUs, JVM, and workload that are used for the baseline:
...
- The baseline line contains 938,563 sampled records: 938,544 loads and 19
  ...
  baseline also ran longer, so it provided Perf with more sampling time. Don't
...
counter line. It doesn't prove that either address belongs to the counter
object. The two addresses don't need to match because they come from separate
JVM processes.
