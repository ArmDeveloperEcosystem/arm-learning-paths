...
generated_summary_faq:
  ...
  summary: >-
    You deploy a three-node Apache Kafka cluster on Arm, beginning with a three-node
    ZooKeeper ensemble. You prepare network ports, configure the brokers, start the services
    in order, and validate end-to-end messaging by creating a topic, producing events, and
    reading them from a client. You also use Terraform and Ansible to automate the same
    topology on AWS Graviton.
  faqs:
  - question: How do I know the ZooKeeper cluster is ready before I configure Kafka?
    answer: >-
      Proceed when all three ZooKeeper nodes are started and reachable from the Kafka nodes. The
  ...
  - question: Which IPs should I put into Kafka’s `server.properties`?
    answer: >-
      ...
      you set up. Use the same addresses you used when bringing up the ZooKeeper cluster.
  - question: Which ports should be open on the nodes?
    answer: >-
      ...
      the rules allow traffic between cluster nodes and from the client where you run Kafka commands.
  - question: What result should I expect after I produce and consume events?
    answer: >-
      ...
      messages confirm the brokers and ZooKeeper are connected and the topic is available.
