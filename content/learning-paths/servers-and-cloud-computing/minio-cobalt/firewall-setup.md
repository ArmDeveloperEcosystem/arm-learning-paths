...
To allow external traffic on port **9000,9001** for MinIO running on an Azure virtual machine, open the port in the Network Security Group (NSG) attached to the virtual machine's network interface or subnet.
...
To expose the TCP port **9000,9001**, create a firewall rule.
...
![Azure portal inbound security rule configuration form showing port 9000,9001, TCP protocol, and Allow action alt-txt#center](images/inbound_rule.png "Network settings")

The network firewall rule is now created, allowing MinIO to communicate over port **9000,9001**.
