...
PAC helps protect return addresses and pointers by adding a cryptographic signature that is checked before the pointer is used, which can detect tampering such as return-oriented programming attempts. 

BTI complements this by restricting where indirect branches are allowed to land, helping prevent attackers from jumping into unintended instruction sequences. 
...
The result is that a fully protected deployment requires both: system libraries compiled with PAC/BTI, and a JVM binary that was itself built with PAC/BTI enabled. You can verify both conditions on a running JVM, which is what the next steps of this Learning Path cover.
