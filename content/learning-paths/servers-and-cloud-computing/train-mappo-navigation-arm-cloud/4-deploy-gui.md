---
title: (Optional) Deploy the checkpoint to a cloud GUI
...
The MARL GUI used in this stage is a separate application from BenchMARL and VMAS. It's not created automatically when you run the training workflow.

The GUI source and helper scripts aren't included in the Arm Learning Paths repository. Treat this stage as optional unless you have the companion checkout described in the [prerequisites](/learning-paths/servers-and-cloud-computing/train-mappo-navigation-arm-cloud/). Checkpoint validation and actor export don't depend on the GUI.

If you already have the companion GUI code, use that checkout and continue with the steps.

If you don't have the GUI code, you can either skip this stage and continue to the [actor-export section](/learning-paths/servers-and-cloud-computing/train-mappo-navigation-arm-cloud/5-export-actor/), or create an equivalent visualization application. A compatible GUI should be able to:
...
The companion GUI also provides the following utilities:
...
The remaining commands assume that the companion GUI is available locally. If you create your own GUI, replace the paths and helper-script names with the corresponding paths in your implementation.
...
The deployment tool performs the following tasks:
...
The deployed model has the following structure:
...
Start the application on the cloud instance. Bind it to the loopback interface so it's not exposed directly through the instance network:
...
Refresh the browser after registration. Select the model in the following order:
...
The following table describes the artifacts that the GUI uses:
...
The full BenchMARL checkpoint is the correct artifact for the GUI. For a downstream inference runtime, export only the actor parameters.
