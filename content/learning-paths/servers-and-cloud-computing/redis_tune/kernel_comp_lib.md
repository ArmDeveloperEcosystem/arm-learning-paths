...
If you build Redis from source, use a recent GCC version from your Linux distribution. If building from source to specifically target the Arm AGI CPU, use GCC version 16.1.0 or later to include optimizations specific to the AGI CPU. Compiler flags such as `-mcpu` and `-flto` can change performance, but different compilers and compiler flags should be tested against your workload before use as they may introduce regressions for your workload. Usage of these flags is explained in the [Migrating C/C++ applications](/learning-paths/servers-and-cloud-computing/migration/c/) section of the [Migrating applications to Arm servers](/learning-paths/servers-and-cloud-computing/migration/) learning path.
