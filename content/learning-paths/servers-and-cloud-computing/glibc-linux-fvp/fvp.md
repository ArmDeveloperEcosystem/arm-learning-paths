...
In addition to the model itself, you also need the device tree that we built previously
and the firmware. To simplify building the firmware, we use a tool called [Shrinkwrap][2].
 ...
 * Paths to the rootfs image, device tree, and kernel image.
