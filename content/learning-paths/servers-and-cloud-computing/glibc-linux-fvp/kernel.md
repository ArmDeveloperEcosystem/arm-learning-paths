...
The Linux kernel image and device tree for the Arm Fixed Virtual Platform (FVP)
model are the first essential components that we need. We are going to build them
from source.
...
The following commands will configure and build the Linux kernel image and the
device tree. All the build output, including the binaries that we intend to use
later, will be put in the `linux-build` subfolder. Run the following commands
...
make -C linux Image dtbs -j $(nproc)
```
...
and `dtbs` targets will produce the binaries that we need.

When the build completes, check that these binaries are present:

```bash
ls linux-build/arch/arm64/boot/dts/arm/fvp-base-revc.dtb
