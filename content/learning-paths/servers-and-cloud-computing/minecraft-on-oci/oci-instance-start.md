...
2. On the Oracle Cloud Infrastructure (OCI) dashboard, go to **Compute > Instances** to start a new instance: 
...
10. Leave the security and storage options as defaults. Then, under **Networking**, update the following:
    ...
    - Select **Automatically assign public IPv4 address** to allow remote access to your Minecraft server instance.
...
2. Under the **Networking** tab, select the subnet name.
   ![OCI instance Networking tab showing the Primary VNIC details and the subnet link to open for security list configuration#center](oci_instance_networking.webp "Open the subnet from the instance Networking tab")
...
4. Under **Security rules**, select **Add Ingress Rules** to add a new ingress rule, then update the following: 
    - Set **Source CIDR** to the public IP address (or range) of the players who'll connect to your server. For example, if your home IP address is `203.0.113.45`, set the Source CIDR to **203.0.113.45/32** to allow only that address. 
    
      You can find your current public IP address by searching "what is my IP address?" in a web browser from the network you'll use to connect. You can add multiple ingress rules if players connect from different networks.

    {{% notice Warning %}}
    Avoid using `0.0.0.0/0` as the **Source CIDR**. `0.0.0.0/0` opens the port to the entire internet and
    exposes your server to unauthorized access attempts. Always restrict access to only the IP addresses that need to connect.
    {{% /notice %}}

    - Set the **Destination Port Range** field to **25565**.
    ![OCI Default Security List Security rules tab showing an ingress rule that allows TCP traffic to destination port 25565 from the selected source CIDR#center](open_minecraft_port.webp "Security list ingress rule for Minecraft traffic on port 25565")
...
![OCI instance Networking tab showing the Public IPv4 address in the Primary VNIC details for SSH access#center](oci_instance_networking.webp "Find the instance public IPv4 address")

Note the IP address under **Public IPv4 address**. 

SSH into your instance, replacing `<path to private key>` with the path to your SSH private key and `<public IPv4 address>` with the instance's public IP:

```bash
ssh -i <path to private key> opc@<public IPv4 address>
```

## What you've accomplished and what's next

You've now provisioned an Ampere A1 virtual machine instance on Oracle Cloud Infrastructure, configured its virtual network to assign a public IP address, opened port `25565`, and connected to the server securely over SSH.
