...
description: Ask Claude Code for an Arm Performix AI insight tied to a specific run and assess whether the response is supported by profile evidence.

layout: learningpathall
---
## Select the exact run

Use a run ID whenever possible. A run ID prevents Claude Code from selecting a similarly named run or a more recent run from a different workload.

If you don't have the ID, ask Claude Code to list supported runs:
...
Claude Code can help with more of this loop if your environment exposes build, deployment, remote-execution, or source-control tools. Review proposed code changes and command approvals before Claude Code applies or runs them.
