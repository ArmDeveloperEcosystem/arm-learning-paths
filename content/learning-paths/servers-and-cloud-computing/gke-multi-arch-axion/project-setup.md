...
- Build **amd64** and **arm64** images with Docker **Buildx (Kubernetes driver)**. The builds run **natively** inside **BuildKit pods** on your GKE node pools (no QEMU emulation).
...
4. Create a **Buildx (Kubernetes driver)** builder that targets both pools, then **build and push multi-architecture images** (amd64 & arm64) natively via BuildKit pods.
...
Enable these APIs so the project can create GKE clusters, push/pull images in Artifact Registry, and use Cloud Build for CI/CD:
...
Create a Docker repository in Artifact Registry (in this region) for pushing/pulling your multi-architecture images:
