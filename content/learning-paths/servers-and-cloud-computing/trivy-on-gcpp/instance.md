...
![Azure Portal VM size selection page showing the D-Series v6 family with D4ps_v6 selected, indicating Arm64 architecture and 4 vCPUs with 16 GiB memory#center](images/instance.png "Select D4ps_v6 from the D-Series v6 family")
...
![Azure Portal inbound port rules section showing HTTP port 80 and SSH port 22 checkboxes selected under Select inbound ports dropdown#center](images/instance1.png "Configure inbound port rules for HTTP and SSH access")

- Now select the **Review + Create** tab and review the configuration for your virtual machine. It should look like the following:

![Azure Portal Review + Create page displaying VM configuration details including Ubuntu Pro 24.04 LTS image, D4ps_v6 size, Arm64 architecture, SSH authentication, and network settings ready for validation#center](images/ubuntu-pro.png "Review VM configuration before creation")

- When you are happy with your selection, select the **Create** button and then **Download Private key and Create Resource** button.

![Azure Portal Generate new key pair dialog showing the Download private key and create resource button with the private key filename displayed#center](images/instance4.png "Download SSH key and create the virtual machine")
...
![Azure Portal deployment complete notification showing green checkmark with Your deployment is complete message, VM resource name, resource group, and public IP address for SSH access#center](images/final-vm.png "Successful VM deployment confirmation")
