...
Embeddings are vectors generated by an AI model. You can convert collections of "tokens" (word fragments) into a point in N-dimensional space.

Then, for any given vector, such as the embedding of a question asked by a user, you can query the vector database to find the embedded data that is most similar.

For this use case, you want to know which Arm learning path is most relevant to a question a user asks.

First, in advance, you have to convert the raw data, which is the Arm learning path content, into more consumable "chunks". In this case, these are small `yaml` files. Then you run those chunks through the LLM model and embed the content into the FAISS vector database.
