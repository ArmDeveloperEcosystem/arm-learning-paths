...
For example, after installing `g++` on Ubuntu 24.04, the default compiler as of May 2026, output appears as follows:
...
If you are using a CPU which has recently been released or about to be released, for example the Arm AGI CPU, we recommend using a recent version of the compiler which has support for that model, as it includes the latest optimizations, support and bug fixes. Due to backwards compatibility of the Arm architecture, older compilers will successfully compile runable binaries but the compiler is unaware of the cost model for that CPU so you may not get the most performant binary. 

For example, according to the gcc documentation, the most recent version, version 16, lists the following support and optimizations in the [release notes](https://gcc.gnu.org/gcc-16/changes.html): 
