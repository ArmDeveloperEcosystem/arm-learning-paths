...
## Use the right compiler version


It's important to use a compiler that provides optimizations for the CPU you are natively compiling on. If you use an older compiler that is not optimized for CPU's architecture, you may not get optimal performance.

For example, the following results were taken on a `r8g.xlarge` instance (Graviton 4, Neoverse V2) running Ubuntu 24.04 LTS, using the default g++ (13.3) and g++ version 9, which does not have optimizations for Neoverse V2. To install g++-9, run:

```bash
sudo add-apt-repository ppa:ubuntu-toolchain-r/test
sudo apt update
sudo apt install g++-9 g++ -y
```
...
In this simple example on AWS Graviton 4, you can see about a 20% speed improvement moving from version 9 to version 13. 
...
Your results may vary depending on the specific CPU architecture and compiler version you use. Performance differences are expected if you run these examples on different Arm platforms or with different compiler toolchains.

{{% /notice %}}

With this combination, you can see that newer compiler versions can impact performance. 
...
## Target balanced performance on AWS Graviton
