...
Collecting samples with perf and preparing a good build to analyse

On Linux, flame graphs are commonly generated from samples collected with `perf`. perf periodically interrupts the running program and records a stack trace, then the collected stacks are converted into a folded format and rendered as the graph. Sampling frequency is important. If the frequency is too low you may miss short lived hotspots, and if it is too high you may introduce overhead or skew the results. To make the output informative, compile with debug symbols and preserve frame pointers so stacks resolve to meaningful function names and unwind reliably. A typical tutorial build uses `-g` and `-fno-omit-frame-pointer`.

Tooling options

The classic workflow is command line driven using perf for collection and separate scripts to generate the flame graph, following the approach introduced by Brendan Gregg, who is widely credited with creating and popularizing flame graphs. Arm has also developed a tool that simplifies this workflow through Arm Total Performance, making it easier to configure collection, run captures, and explore the resulting call hierarchies without manually stitching together the individual steps. 
