...
The Profiler can be used in the editor and with your connected Android device. It is always recommended to measure your actual target as your development environment may be very different. Using the Profiler when running your app in the editor can be useful, especially if your performance issues are general or simple to identify. Although, generally speaking, profiling on an actual target device will always provide more accurate results.
...
In the sample, the performance issues will be apparent soon after launch. To profile directly on an Android device:
...
4. Choose your save path and name

5. Wait for Unity to launch the collision sample and auto-connect the Profiler

6. Once the Profiler is connected, the Profiler window will open and the _CPU Usage_ graph will start to fill
...
2. A round record button is present in the top-left.

3. To the left of the record button is a drop-down menu identifying the connection mode and device. To profile the app running in the editor, you would set this to _Play Mode_.

4. Your Android device should appear in the list. Select it to connect the Profiler.

5. Press the record button to start profiling (the button will turn red).
...
The CPU Usage graph provides a very high level view of what is happening in the app and can help you decide if the performance issue is CPU or GPU related. But where exactly is the problem? With the collision sample you can tell that something is wrong on the CPU but with what bit of code?
...
For now, focus on the main thread because it seems to be very busy. The top row says that the PlayerLoop entry is taking 56.75ms. Even if nothing else was running, you would be limited to approximately 17 frames per second (1000 / 56.75 = 17.6). The bottleneck seems to be CPU related.
...
You may also nest tagged samples inside other samples in order to organize your profile data into a Hierarchy (as shown in the Timeline and Hierarchy views).
