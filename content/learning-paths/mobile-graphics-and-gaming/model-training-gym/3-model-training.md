...
In this section, you'll get hands-on with how you can use Model Gym to train the NSS use case.
...
|--------------|----------------|--------------------------------------------------------------------------|
| PyTorch      | `.pt`            | training, fine-tuning, or evaluation in notebooks or scripts using Model Gym  |
| VGF          | `.vgf`           | for deployment using ML Extensions for Vulkan on Arm-based hardware or emulation layers |

Both formats are available in the [NSS repository on Hugging Face](https://huggingface.co/Arm/neural-super-sampling). You'll also be able to explore configuration files, model metadata, usage details, and detailed documentation on the use case.
...
In this part of the Learning Path, you will run through two Jupyter notebooks. Return to this tutorial when you're done to continue to quantization-aware fine-tuning and export.
{{% /notice %}}

You will become familiar with the following steps:
...
```output
neural-graphics-model-gym-examples/tutorials/model_training_example.ipynb
```
...
```output
neural-graphics-model-gym-examples/tutorials/model_evaluation_example.ipynb
```
