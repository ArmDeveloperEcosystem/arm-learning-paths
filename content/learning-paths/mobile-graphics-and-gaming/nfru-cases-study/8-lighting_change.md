...
Lighting changes matter because optical flow is estimated from image changes. If color changes because geometry moved, optical flow can help interpolation. If color changes mainly because illumination changed, optical flow may interpret that lighting change as motion. This can fail to line up with depth and motion vectors, which describe geometry movement. Dramatic changes, such as one source frame being dark and the next being bright, are more likely to produce ghosting, flicker, or stale bright and dark remnants.
...
When this type of artifact appears, inspect both the image inputs and the motion/depth inputs:
