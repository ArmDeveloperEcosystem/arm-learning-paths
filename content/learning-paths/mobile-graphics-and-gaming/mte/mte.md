...
description: "Build and run a C program on AArch64 Linux to observe how Arm Memory Tagging Extension detects invalid memory accesses."
...
This occurs because `getauxval()`, the function to query hardware features from applications, does not report the `HWCAP2_MTE` field as set. This means MTE is not implemented in the processor. For more information about how to identify hardware features, see [ARM64 ELF hwcaps](https://docs.kernel.org/arch/arm64/elf_hwcaps.html).
...
Notice that the tag value will be different from the example output (the tag value is 1). Each time you run the application, a random tag will be generated by the `IRG` instruction.

You have learned how to build and run a program with the Memory Tagging Extension on Linux. To continue learning about MTE, see the further-reading resources on the overview page.
