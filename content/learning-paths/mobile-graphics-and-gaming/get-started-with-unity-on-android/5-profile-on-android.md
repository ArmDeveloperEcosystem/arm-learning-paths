...
1. To activate profiling, you will enable the _Development Build_ and _Autoconnect Profiler_ options.
...
Remembering that you're trying to find out why the spinning cube isn't running smoothly, there are a few things that stand out in the timeline:

- In the main timeline at the top, the different areas of the app are colored. The graph is filled with a lot of blue. On the left under _CPU Usage_ you can see that blue is for _Scripts_.

- Unity has provided convenient figures for you to compare against in the main timeline (on the left, you see tags showing 60FPS, 30FPS and 15FPS). It is telling you that in order to hit 60 frames per second (FPS), your frame needs to take no more than 16 milliseconds (ms). To hit 30 frames per second, your frame needs to take less than 33ms, and less than 66ms if you targeted a frame rate of 15FPS.

- Looking at the _Main Thread_ for the selected frame, you can see that the _PlayerLoop_ is taking _117.7ms_. That is awfully high and way above the targeted frame time of 16ms. Part of the high level _PlayerLoop_ task is `Update.ScriptRunBehaviourUpdate` which calls the _Update_ function on all of the scripts in the app. The last entry is _Spin.Update_.

- _Spin.Update_ is the _Update_ function in the _Spin_ component. On the frame selected it took over _100ms_. Even with just this function alone, the maximum frame rate would be something like 10FPS (one second or 1000ms, divided by 100ms, is 10).

So by using the Profiler window, you now know where to look in the next step of your investigation.
