...
def extract_sentences_with_matches(text, pattern):
    import re
...
def apply_manual_review_rules(text, rules):
    suggestions = []
    for rule in rules:
        pattern = re.compile(rule["pattern"])
        matches = pattern.finditer(text)
        for match in matches:
            sentence = extract_sentences_with_matches(text, rule["pattern"])
            suggestions.append({
                "match": match.group(),
...
def run_manual_review_interactively(text, rules):
    updated_text = text
    manual_suggestions = apply_manual_review_rules(text, rules)
    for suggestion in manual_suggestions:
        for sentence in suggestion["sentence"]:
            print(f"\nContext: {sentence}")
            print(f"Suggestion: Replace '{suggestion['match']}' with '{suggestion['replacement']}'")
            print(f"Reason: {suggestion['reason']}")
            choice = input("Apply this change? (y/n): ").strip().lower()
            if choice == 'y':
                updated_text = updated_text.replace(suggestion["match"], suggestion["replacement"], 1)
    return updated_text
